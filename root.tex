...
\thanks{$^{1}$Luigi Franco Tedesco is an undergraduate student with Faculty of Electrical Engineering, Robotics and Artificial Intelligence,
        ...
        {\tt\small tedesco@ensta-paristech.fr}}%
...
\thanks{$^{3}$Jean-Fran\c{c}ois Goudou is with the Department of Electrical Engineering, Thales Theresis,
...
The viterbi algorithm is used to find the most likely path taking into account all possible states transitions and the probability of being recognized by single view pipeline. The viterbi path coincides with the sequence of recognized view for each observation. \ref{eq:viterbi}.

\begin{equation}
  \begin{array}{rcl}
    V_{1,k} &=& \textbf{p}_{i}^e \cdot \pi_k \\
    V_{t,k} &=& \max_{x \in S} \left(  \textbf{p}_{i}^e \cdot \textbf{p}_{ij}^{t} \cdot V_{t-1,x}\right)
  \end{array}
	\label{eq:viterbi}
\end{equation}
